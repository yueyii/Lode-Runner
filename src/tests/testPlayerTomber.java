...
public class testPlayerTomber extends AbstractRunnerTest{
		...
		public void testInitPreCasPos() throws Exception{
			try {
				...
				getGuard(guard2).init(getEnvi(), 27 , 12, 1);
				getItem(item).init(getEnvi(), 20 , 9, 0, ItemType.TREASURE);
				getItem(item2).init(getEnvi(), 19 , 9, 1, ItemType.TREASURE);
				getItem(item3).init(getEnvi(), 24 , 9, 2, ItemType.ARMS);
